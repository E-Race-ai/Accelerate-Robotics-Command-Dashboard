...
// WHY: Logos are uploaded through a multer filter that whitelists these MIME
// types — explicit ext → MIME mapping avoids any sniff-based fallback.
...
router.post('/:slug/sign-in', magicLinkLimit, async (req, res, next) => {
  try {
    ...
    if (!portal || portal.status === 'archived') {
      // Don't leak whether the portal exists; return 200 either way.
    ...
    // Don't disclose whether the email is invited — return 200 regardless.
...
// Magic-link callback — typically hit via GET so the link in the email Just Works.
router.get('/:slug/auth', async (req, res, next) => {
  try {
    ...
    // Sign-in successful → land on the portal home.
    res.redirect(`/portal/${encodeURIComponent(portal.slug)}/`);
  } catch (err) { next(err); }
});

// Sign out — accepts the URL slug for consistency, no auth required (idempotent).
router.post('/:slug/sign-out', (req, res) => {
  clearSessionCookie(res);
  res.json({ signed_out: true });
});

// Public logo. Customers see this on the sign-in page BEFORE authenticating,
// so the route deliberately does NOT require a portal session.
router.get('/:slug/logo', async (req, res, next) => {
  try {
    const portal = await getPortalBySlug(req.params.slug);
    if (!portal || !portal.customer_logo_path) {
      return res.status(404).json({ error: 'No logo' });
    }
    const absolutePath = resolveStoredPath(portal.customer_logo_path);
...
// Portal metadata (welcome, theme, permissions) — what the portal home page needs.
...
// Update display name (one-time, on first login).
...
router.post('/:slug/content', requirePortalSession, async (req, res, next) => {
  // Customer uploads — gated by portal setting.
...
// Mark an item as viewed (called when the customer opens the file viewer).
...
// Download a file. Gated by allow_external_downloads. Returns the file stream.
...
router.get('/:slug/participants', requirePortalSession, async (req, res, next) => {
  try {
    ...
    // Strip fields that customers don't need to see.
...
router.post('/:slug/participants', requirePortalSession, async (req, res, next) => {
  // Customer-side invites are gated by portal setting.
