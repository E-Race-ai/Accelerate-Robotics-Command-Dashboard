 ...
 *
 * Wraps all SQLite queries for the portal feature. Uses the project's existing
 * { one, all, run, transaction } helpers from src/db/database.js so query
...
async function listPortals({ includeArchived = false } = {}) {
  const sql = includeArchived
    ? `SELECT * FROM portal_spaces ORDER BY created_at DESC`
    : `SELECT * FROM portal_spaces WHERE archived_at IS NULL ORDER BY created_at DESC`;
...
async function getPortalBySlug(slug) {
  const row = await one(`SELECT * FROM portal_spaces WHERE slug = ?`, [slug.toLowerCase()]);
  return row ? rowToPortal(row) : null;
}
...
async function createPortal({
  ...
  await run(
    `INSERT INTO portal_spaces
     ...
     VALUES (?, ?, ?, ?, ?, ?, ?)`,
    [id, slug.toLowerCase(), name, customer_name, deal_id, String(owner_user_id), welcome_message],
  );
...
async function inviteParticipant({ portalId, email, role = 'external', invitedBy }) {
  ...
  await run(
    `INSERT INTO portal_participants (id, portal_id, email, role, invited_by)
     VALUES (?, ?, ?, ?, ?)`,
    [id, portalId, email.toLowerCase(), role, String(invitedBy)],
  );
...
async function touchParticipant(id) {
  await run(
    `UPDATE portal_participants SET last_seen_at = datetime('now') WHERE id = ?`,
    [id],
  );
}
...
async function consumeMagicToken(rawToken) {
  ...
  const tokenHash = crypto.createHash('sha256').update(rawToken).digest('hex');
  const row = await one(
    `SELECT t.id AS token_id, p.id AS pid, p.portal_id, p.email, p.role, p.status
