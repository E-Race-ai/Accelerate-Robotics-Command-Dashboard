...
### Added
- **WiFi Vision Extreme launcher + one-line installer** (2026-05-08): The I.T. tile at `/pages/wifi-vision.html` (added in #211) was a blank page in production because the embedded iframe pointed at `http://127.0.0.1:8765` and HTTPS pages can't iframe HTTP. Replaced with a branded launcher card (animated mesh background, frosted-glass card, pulsing WiFi rings, sweeping scan-line) plus a one-line `curl … | bash` installer. First click on **Start Live Cast** / **Start Scan** copies the install command to the user's clipboard and shows a setup hint; pasting the line in Terminal downloads `installers/wifi-vision-bundle.tar.gz`, sets up a Python venv at `~/Library/Application Support/WiFiVisionExtreme/`, registers a launchd LaunchAgent so the backend auto-starts on login, then uses AppleScript to navigate the user's existing browser tab (Safari, Chrome, Brave, Edge, Arc all supported) back to the live dashboard — same tab, no new windows. Installer + bundle are served publicly from `/installers/*` so the curl call doesn't need an auth cookie.
- **Mobile Device Management (MDM)** (2026-05-07): new I.T.-department toolkit module backed by Google's Android Management API for managing Keenon-style robot tablets and Atlas-owned staff Android devices. New tile at `/pages/mdm.html`, native Express routes at `/api/mdm/*` (enrollment tokens, devices, lock/reboot commands, policy), AMAPI client wrapper at `src/services/mdm-amapi.js`. Auth uses the dashboard's existing JWT cookie. Configured via `AMAPI_ENTERPRISE_NAME` / `AMAPI_POLICY_ID` / `GOOGLE_APPLICATION_CREDENTIALS` env vars (see `docs/30-integrations/mdm-android.md` for full setup including the Workspace gotcha and the non-Workspace Gmail EMM-identity workaround). Single-policy mode for now; multi-policy and device tagging deferred to v0.2. New `googleapis` + `qrcode` npm dependencies.
