...
- **System settings table + Settings → System tab** (2026-05-01): admins can paste the current Creative Labs cloudflared tunnel URL via `/admin/settings`. Backed by a new `system_settings` SQLite table (key/value with `updated_at` and `updated_by`), exposed via `GET/PUT /api/system-settings/:key` (auth required, key whitelisted). The Robot Command and Beam Bot Playground embed pages now fetch this URL on load and probe it before showing the iframe — if not set or unreachable, they show a fallback panel pointing to the Settings tab. CSP `frame-src` and `connect-src` extended to `https://*.trycloudflare.com` so the iframe and probe work without further changes when the tunnel rotates.
